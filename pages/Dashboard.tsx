...
const Dashboard: React.FC = () => {
  ...
  const pendingOrders = packing_orders?.filter(o => o.status === 'Open' || !o.status).length || 0;
  const completedOrders = packing_orders?.filter(o => o.status === 'Completed').length || 0;
  ...
  // Chart Data - Production by Machine (simplified)
                ...
                <tbody className="divide-y divide-slate-100">
                    {molding_logs?.slice(0, 5).map((log) => (
                        <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                            ...
                            <td className="px-6 py-4 text-slate-700">{log.productName}</td>
                            ...
                            <td className="px-6 py-4">
                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    ${log.status === 'เสร็จสิ้น' || log.status === 'Completed' ? 'bg-green-100 text-green-800' : 
                                      log.status === 'In Progress' || log.status === 'ทำงาน' ? 'bg-blue-100 text-blue-800' : 
