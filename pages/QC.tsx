  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500">Inspect and approve production batches.</p>
      </div>
        ...
        {sortedQc.map((entry) => (
          <div key={entry.id} className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex flex-col">
            ...
            <div className="text-sm text-slate-500 mb-4">
              <span className="block">Qty: <span className="font-mono text-slate-700 font-medium">{entry.quantity} {entry.unit}</span></span>
              <span className="block text-xs mt-1">Source Date: {entry.sourceDate}</span>
            </div>
            ...
            <div className="mt-auto pt-4 border-t border-slate-100 flex gap-2">
                {entry.status === 'Pending' ? (
                    ...
                    <div className="w-full text-center text-xs text-slate-400 italic">
                        Inspected by {entry.qcInspector || 'System'} on {entry.qcDate}
                    </div>
                )}
            </div>
          </div>
