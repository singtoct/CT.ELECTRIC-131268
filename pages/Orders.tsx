  ...
  const planningData = useMemo(() => {
      ...
      production_documents.forEach(doc => {
          ...
          doc.items.forEach(item => {
              ...
              productMap[item.productName].orders.push({ docId: doc.id, docNumber: doc.docNumber, qty: item.quantity, dueDate: item.dueDate, status: doc.status });
          });
      });
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-black text-slate-800 tracking-tight">{t('mrp.title')}</h2>
          <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[4px] mt-1">{t('mrp.subtitle')}</p>
        </div>
        <div className="bg-white p-2 rounded-xl border border-slate-200 shadow-sm flex items-center gap-2 text-slate-500 text-xs font-bold">
            <Calculator size={16}/> <span>{t('mrp.autoCalc')}</span>
        </div>
      </div>
          ...
          {planningData.map((plan, idx) => {
              ...
              return (
                  <div key={idx} className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden group hover:border-blue-200 transition-all">
                      <div className="p-6 cursor-pointer" onClick={() => toggleProductExpand(plan.productName)}>
                          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                              ...
                              <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 text-center">
                                  <div className="flex flex-col p-2 rounded-xl bg-slate-50/50"><span className="text-[9px] font-black text-slate-400 uppercase tracking-wider">{t('mrp.demand')}</span><span className="text-lg font-black text-slate-800">{plan.totalDemand.toLocaleString()}</span></div>
                                  <div className="flex flex-col p-2 rounded-xl bg-blue-50/50 border border-blue-100/50"><span className="text-[9px] font-black text-blue-400 uppercase tracking-wider">{t('mrp.stock')}</span><span className="text-lg font-bold text-blue-600">{plan.stock.toLocaleString()}</span></div>
                                  <div className="flex flex-col p-2 rounded-xl bg-amber-50/50 border border-amber-100/50"><span className="text-[9px] font-black text-amber-400 uppercase tracking-wider">{t('mrp.wip')}</span><span className="text-lg font-bold text-amber-600">{plan.wip.toLocaleString()}</span></div>
                              ...
                              <div className="text-slate-300 group-hover:text-slate-500">{isExpanded ? <ChevronUp /> : <ChevronDown />}</div>
                          </div>
                      </div>
                      {isExpanded && (
                          <div className="bg-slate-50/50 border-t border-slate-200 p-6">
                              <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2"><LayoutList size={14}/> {t('mrp.orderList')}</h4>
                                  ...
                                  {plan.orders.map((order, oIdx) => (
                                      <div key={oIdx} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
                                          ...
                                          <div className="space-y-1 mb-3">
                                              <div className="flex justify-between text-xs"><span className="text-slate-400 font-bold">{t('mrp.demand')}:</span><span className="font-black text-slate-800">{order.qty.toLocaleString()}</span></div>
                                              <div className="flex justify-between text-xs"><span className="text-slate-400 font-bold">Due Date:</span><span className="font-medium text-slate-600">{order.dueDate}</span></div>
                                          </div>
                                      </div>
                                  ))}
                              </div>
