  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 font-bold text-xs uppercase tracking-widest mt-1">Real-time Stock Management</p>
      </div>
      ...
      <div className="flex bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm w-fit">
        <button onClick={() => setActiveTab('finished')} className={`px-6 py-3 rounded-xl font-black text-xs uppercase tracking-wider transition-all flex items-center gap-2 ${activeTab === 'finished' ? 'bg-slate-100 text-slate-800 shadow-inner' : 'text-slate-400 hover:text-slate-600'}`}>
          <Archive size={16} /> สินค้าสำเร็จรูป
        </button>
        <button onClick={() => setActiveTab('component')} className={`px-6 py-3 rounded-xl font-black text-xs uppercase tracking-wider transition-all flex items-center gap-2 ${activeTab === 'component' ? 'bg-slate-100 text-slate-800 shadow-inner' : 'text-slate-400 hover:text-slate-600'}`}>
          <GitMerge size={16} /> ชิ้นส่วนประกอบ
        </button>
        <button onClick={() => setActiveTab('raw')} className={`px-6 py-3 rounded-xl font-black text-xs uppercase tracking-wider transition-all flex items-center gap-2 ${activeTab === 'raw' ? 'bg-slate-100 text-slate-800 shadow-inner' : 'text-slate-400 hover:text-slate-600'}`}>
          <Layers size={16} /> วัตถุดิบจัดซื้อ
        </button>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center">
            <div className="relative w-full max-w-md">
                ...
                <input type="text" placeholder="ค้นหาชื่อรายการ..." className="pl-12 pr-6 py-3.5 border border-slate-200 rounded-2xl w-full !bg-white !text-slate-900 font-bold focus:ring-4 focus:ring-primary-50 transition-all outline-none shadow-sm" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
        </div>
        <table className="w-full text-sm text-left">
          <thead className="bg-slate-50 text-slate-400 font-black border-b border-slate-200 uppercase tracking-[2px] text-[10px]">
            <tr>
              <th className="px-8 py-5">ชื่อรายการ (Item Name)</th>
              <th className="px-6 py-5 text-right">จำนวนคงคลัง</th>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in duration-200">
                <div className="px-10 py-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                    <h3 className="text-2xl font-black text-slate-800 tracking-tight">ปรับปรุงยอดสต็อก</h3>
                ...
                <div className="p-10 space-y-6">
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[3px] mb-2">ชื่อรายการ</label>
                        <div className="text-xl font-black text-slate-800">{editingItem.name}</div>
                    </div>
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[3px] mb-2">จำนวนใหม่ ({editingItem.unit})</label>
                ...
                <div className="px-10 py-8 bg-slate-50 border-t border-slate-100 flex gap-4">
                    <button onClick={() => setEditingItem(null)} className="flex-1 py-4 text-slate-500 font-black hover:bg-slate-200 rounded-2xl transition-all">ยกเลิก</button>
                    <button onClick={handleUpdateStock} className="flex-1 py-4 bg-primary-600 text-white font-black rounded-2xl shadow-xl shadow-primary-600/20 hover:bg-primary-700 transition-all flex items-center justify-center gap-2"><Save size={20}/> บันทึกยอด</button>
                </div>
            </div>
        </div>
      )}
